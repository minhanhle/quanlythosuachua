...
function fixhomeService($http, $log) {
	...
	this.saveFixer = function (data) {
		...
		if (data.hinhanh) {
			return $http.post('/saveImage', dataImage).then(
				function successCallback(response) {
					data.hinhanh = response.data;
					return $http.post('/tho', data).then(
						function successCallback(resFixer) {
							return "Thêm thành công."
						},
						function errorCallback(resFixer) {
							return "Vui lòng kiểm tra lai các thông tin."
						})
				},
				function errorCallback(response) {
	...
	this.updateFixer = function (data) {
		...
		if (data.hinhanh && data.hinhanh.indexOf("base64") > -1) {
			return $http.post('/saveImage', dataImage).then(
				function successCallback(response) {
					data.hinhanh = response.data;
				...
				function errorCallback(response) {
					return "Kiểm tra lại dung lượng hình ảnh."
				}
			)
		} else {
			data.hinhanh = "images-fixer/fixer.png";
			return $http.put('/tho/' + data.cmnd, data).then(
				function successCallback(response) {
					return "Cập nhật thành công."
				},
			...
			$http.post('/sendmail', body).then(
				function successCallback(response) {
					if (data.quyen === 'tho') {
						...
						return $http.put('/tho/' + data.cmnd, val).then(function (resUpdate) {
						...
						return $http.put('/nhanvienvanphong/' + data.cmnd, val).then(function (resUpdate) {
							return "Tạo tài khoản thành công!"
						})
					}
	...
	this.saveOfficer = function (data) {
		if (data.hinhanh) {
			...
			return $http.post('/saveImage', dataImage).then(
				function successCallback(response) {
					data.hinhanh = response.data;
					$http.post('/nhanvienvanphong', data).then(
						function successCallback(resOff) {
							return "Thêm thành công."
						},
						function errorCallback(response) {
							return "Vui lòng kiểm tra lai các thông tin."
		...
		return $http.get('/nhanvienvanphong').then(function (response) {
			...
			$http.get('/lichnghitheothang?ngay=' + ngay).then(function (resDayOff) {
				...
				for (var l = 0; l < officers.length; l++) {
					...
					if (tgbatdauLamViec < firstDay.getTime() || (tgbatdauLamViec > firstDay.getTime() && tgbatdauLamViec < lastDay.getTime()) && nhanvien.xacnhan) {
						for (var k = 0; k < resDayOff.data.length; k++) {
							var part = [];
							var lichban = resDayOff.data[k];
					...
					if (!nhanvien.xacnhan) {
						nhanvien.songaylamviec = 0;
						nhanvien.luong = 0;
					}

					daysOf = 0;
					var currentMonth = new Date().getMonth() + 1;
					if (parseInt(thang, 10) === parseInt(currentMonth, 10)) {
						var date = new Date();
						var currentDay = date.getDate();
						var lastDay = new Date(date.getFullYear(), date.getMonth() + 1, 0).getDate();

						daysOf = lastDay - currentDay;

						if ((parseInt(nhanvien.songaynghi, 10) + daysOf) > 8) {
							daysOf = daysOf - 8;
						} else {
							daysOf = -parseInt(nhanvien.songaynghi, 10);
						}
					}

					if (tgbatdauLamViec > firstDay.getTime() && tgbatdauLamViec < lastDay.getTime()) {
						nhanvien.luong = (nhanvien.luongtheothang / nhanvien.songaylamviec) * (nhanvien.songaylamviec - nhanvien.songaynghi - nhanvien.ngaybatdaulamviec.getDate() - daysOf);
					} else if (nhanvien.xacnhan) {
						nhanvien.luong = (nhanvien.luongtheothang / nhanvien.songaylamviec) * (nhanvien.songaylamviec - nhanvien.songaynghi - daysOf);
					}
	...
	this.loadListRequest = function () {
		return $http.get('/yeucau').then(function (response) {
			...
			for (var j = 0; j < response.data.length; j++) {
				...
				data.ngaydatyeucau = new Date(moment(data.ngaydatyeucau));
	...
	this.getRequestCus = function (item) {
		return $http.get('/yeucauKH/' + item).then(
			function successCallback(response) {
				var result = [];
				for (var j = 0; j < response.data.length; j++) {
					var data = response.data[j];
	...
	this.deleteRequestbyID = function (item) {
		return $http.delete('/yeucau/' + item).then(
			function successCallback(response) {
				return $http.delete('/lichlamviectho/' + item).then(
					function successCallback(response) {
						return response.data;
					},
					function errorCallback(response) {
	...
	this.saveSkill = function (data) {
		if (data.hinhanh) {
			...
			data.hinhanh = "images-fixer/suachuacacloai.png";
			return $http.post('/dichvu', data).then(
				function successCallback(response) {
					return "Thêm thành công."
				},
	...
	this.getcustomerbyAccount = function (account) {
		return $http.get('/khachhangtk/' + account).then(
			function successCallback(response) {
				return response.data;
			},
			function errorCallback(response) {
