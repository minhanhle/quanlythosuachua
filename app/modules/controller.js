...
function navBarCtrl($scope, $mdSidenav, $state, $log, $interval, fixhomeService, $mdDialog, $mdMedia, $rootScope, $cookies) {
	...
	if ($rootScope.user) {
		...
		if (window.location.href === "http://localhost:8000/") {
			if ($rootScope.user.quyen === 'nhanvienxl') {
				$state.go('yeucau');
			} else if ($rootScope.user.quyen === "nhanvienkt") {
				$state.go('nhanvien');
			} else
				$state.go('home');
		}
	...
	vm.modelRequest = function () {
		if ($rootScope.user.quyen !== "nhanvienkt")
			$state.go('yeucau');
	}
...
function nhanvienCtrl($state, $mdDialog, fixhomeService, $http, $scope, $log, $rootScope, $cookies, $mdMedia, fileReader, $http) {
	var vm = this;
	vm.newImg = false;
	vm.selected = [];
	vm.tho = [];
	...
	vm.saveImage = function () {
		data = {
			hinhanh: $scope.myCroppedImage
		}
		$http.post('/saveImage', data).then(
	...
	vm.themnv = function () {
		if (vm.newImg) {
			$scope.data.hinhanh = $scope.myCroppedImage;
		}
	...
	vm.capnhatnv = function () {
		if (vm.newImg) {
			vm.nv.hinhanh = $scope.myCroppedImage;
		}
	...
	vm.capnhatnvvp = function () {
		if (vm.newImg) {
			vm.nvvp.hinhanh = $scope.myCroppedImage;
		}
...
function lichbanthoCtrl($state, $mdDialog, fixhomeService, $log, $scope, $mdMedia, $rootScope, $filter, $http, $q, $timeout, moment, calendarConfig) {
	...
	vm.luulichnghitho = function () {
		vm.value.cmnd = $state.params.cmnd ? $state.params.cmnd : $rootScope.cmndT;
	...
	vm.loadEvents = function () {
		vm.events = [];
		vm.cmndT = $state.params.cmnd ? $state.params.cmnd : $rootScope.cmndT;
		fixhomeService.requestOfFixer(vm.cmndT).then(function (res) {
			$log.info(res);
			for (var i = 0; i < res.length; i++) {
				var type, name, obj;
				if (res[i].trangthai === "Bắt đầu") type = calendarConfig.colorTypes.info;
				if (res[i].trangthai === "Xác nhận") type = calendarConfig.colorTypes.warning;
				if (res[i].trangthai === "Từ chối") type = calendarConfig.colorTypes.special;
		...
		fixhomeService.getDayOfFixerbyId(vm.cmndT).then(function (response) {
			for (var i = 0; i < response.length; i++) {
				var type1;
				var obj1;
				type1 = calendarConfig.colorTypes.inverse;

				var bd = response[i].giobatdau.getHours() + ":" + response[i].giobatdau.getMinutes();
				var kt = response[i].gioketthuc.getHours() + ":" + response[i].gioketthuc.getMinutes();
...
function yeuCauCtrl($state, $mdDialog, fixhomeService, $log, $scope, $mdMedia, $rootScope, $cookies, $filter) {
	...
	vm.trangthai = ["Bắt đầu", "Xác nhận", "Từ chối", "Đang thực hiện", "Kết thúc"];
	...
	$scope.showlichTho = function (ev, cmndTho) {
		$rootScope.cmndT = cmndTho;
		var useFullScreen = ($mdMedia('sm') || $mdMedia('xs')) && $scope.customFullscreen;
		$mdDialog.show({
				controller: lichbanthoCtrl,
				templateUrl: 'views/nhanvien-showlich.html',
				parent: angular.element(document.body),
				targetEvent: ev,
				clickOutsideToClose: true,
				fullscreen: useFullScreen
	...
	var checkSkil = function (arr1, arr2) {
		...
		if(dem !== arr1.length || dem!==arr2.length) return false;
		return true;
	}
	...
	vm.modelCust = function (item) {
		fixhomeService.getcustomerbyAccount(item).then(function (resCus) {
	...
	vm.dangnhap = function () {
		if ($scope.user) {
			fixhomeService.login($scope.user).then(function (response) {
				if (!response || response.quyen === 'tho') {
...
function caNhanCtrl($state, $mdDialog, fixhomeService, $log, $scope, $mdMedia, $rootScope) {
	...
	vm.select = true;
	vm.khuvuc = ["Trung tâm", "Đông", "Tây", "Nam", "Bắc"];
	vm.trangthai = ["Bắt đầu", "Xác nhận", "Tự chối", "Đang thực hiện", "Kết thúc"];
	if (!$rootScope.user) {
		$state.go('login');
	}
...
function dichvuCtrl($state, $mdDialog, fixhomeService, $log, $scope, $mdMedia, $rootScope) {
	...
	vm.themdv = function (ser) {
		if (vm.newImg) {
			ser.hinhanh = $scope.myCroppedImage;
		}
...
function khachhangCtrl($state, $mdDialog, fixhomeService, $log, $scope, $mdMedia, $rootScope, $cookies) {
	...
	$scope.showListRequest = function (ev, cus) {
		$rootScope.cus = cus;
		var useFullScreen = ($mdMedia('sm') || $mdMedia('xs')) && $scope.customFullscreen;
		$mdDialog.show({
				controller: khachhangCtrl,
				controllerAs: "vm",
				templateUrl: 'views/khachhang-showyeucau.html',
				parent: angular.element(document.body),
				targetEvent: ev,
				clickOutsideToClose: true,
				fullscreen: useFullScreen
	...
	if(vm.cus) {
		fixhomeService.getRequestCus(vm.cus.taikhoan).then(function(resReq) {
			vm.yeucauKH = resReq;
		})
	}
...
function thongkeCtrl($state, $mdDialog, fixhomeService, $log, $scope, $mdMedia, $rootScope, $filter, $http, $q, moment, calendarConfig) {
	...
	$scope.chartCostService = {};

	$scope.chartCostService.type = "PieChart";

	$scope.chartQuan = {};

	$scope.chartQuan.type = "PieChart";

	$scope.chartCostDistrict = {};

	$scope.chartCostDistrict.type = "PieChart";

	$scope.chartDay = {};

	$scope.chartDay.type = "ColumnChart";

	$scope.chartTime = {};

	$scope.chartTime.type = "LineChart";
		...
		fixhomeService.dashboard(vm.start, vm.end).then(function (res) {
			fixhomeService.getSkill().then(function (response) {
				...
				for (var n = 0; n < res.length; n++) {
					for (var l = 0; l < vm.dsdv.length; l++) {
						...
						if (dvyc.indexOf(vm.dsdv[l].tenDichVu) !== -1) {
							vm.dsdv[l].dem++;
							vm.dsdv[l].doanhthu += res[n].phidichvu;
						}
					}
				}
				...
				for (var i = 0; i < vm.dsdv.length; i++) {
					...
					dv.push({
						"v": vm.dsdv[i].doanhthu
					});

					$scope.chartCostService.data.rows.push({
						"c": dv
					});
				}
			...
			fixhomeService.getDistrict().then(function (response1) {
				...
				for (var i = 0; i < vm.quans.length; i++) {
					...
					q.push({
						"v": vm.quans[i].doanhthu
					});

					$scope.chartCostDistrict.data.rows.push({
						"c": q
					});
				}
			...
			for (var k = 0; k < res.length; k++) {
				...
				for (var i = 0; i < $scope.statusReq.length; i++) {
					if ($scope.statusReq[i].name === res[k].trangthai) {
						$scope.statusReq[i].count++;
					}
				}
			}

			for (var g = 0; g < $scope.data.length; g++) {
				var timeReq = [];
				timeReq.push({
					"v": $scope.labels[g]
				});
				...
				$scope.chartTime.data.rows.push({
					"c": timeReq
				});
			}
			...
			$scope.chartTime.options = {
				'title': 'Thống kê yêu cầu theo thời gian thực hiện từ ' + vm.start.getDate() + "/" + (vm.start.getMonth() + 1) + "/" + vm.start.getFullYear() + " đến " + vm.end.getDate() + "/" + (vm.end.getMonth() + 1) + "/" + vm.end.getFullYear()
			};
			...
			$scope.chartCostDistrict.options = {
				'title': 'Thống kê doanh thu yêu cầu theo quận từ ' + vm.start.getDate() + "/" + (vm.start.getMonth() + 1) + "/" + vm.start.getFullYear() + " đến " + vm.end.getDate() + "/" + (vm.end.getMonth() + 1) + "/" + vm.end.getFullYear()
			};

			$scope.chartCostService.options = {
				'title': 'Thống kê doanh thu yêu cầu theo dịch vụ từ ' + vm.start.getDate() + "/" + (vm.start.getMonth() + 1) + "/" + vm.start.getFullYear() + " đến " + vm.end.getDate() + "/" + (vm.end.getMonth() + 1) + "/" + vm.end.getFullYear()
			};
	...
	$scope.readyHandlerbyTime = function (chartWrapper) {
		vm.chartbyTime = chartWrapper.getChart().getImageURI();
	}

	$scope.readyHandlerbyCostService = function (chartWrapper) {
		vm.chartbyCostService = chartWrapper.getChart().getImageURI();
	}
	$scope.readyHandlerbyCostDistrict = function (chartWrapper) {
		vm.chartbyCostDistrict = chartWrapper.getChart().getImageURI();
	}

	vm.generatePDF = function () {
		var doc = new jsPDF();
		doc.setFontSize(16);
		doc.text(70, 18, "Report (" + vm.start.getDate() + "/" + (vm.start.getMonth() + 1) + "/" + vm.start.getFullYear() + "-" + vm.end.getDate() + "/" + (vm.end.getMonth() + 1) + "/" + vm.end.getFullYear() + ")");
		doc.setFontSize(14);

		doc.text(18, 28, "1. Services on requests");
		doc.addImage(vm.chartbyService, 'JPEG', 20, 30, 100, 70);
		doc.addImage(vm.chartbyCostService, 'JPEG', 110, 32, 100, 70);

		doc.text(18, 100, "2. Districts on requests");
		doc.addImage(vm.chartbyDistrict, 'JPEG', 20, 102, 100, 70);
		doc.addImage(vm.chartbyCostDistrict, 'JPEG', 110, 102, 102, 70);

		doc.text(18, 175, "3. Dates on requests");
		doc.addImage(vm.chartbyDate, 'JPEG', 30, 177, 150, 60);
		doc.addImage(vm.chartbyTime, 'JPEG', 30, 230, 150, 60);
